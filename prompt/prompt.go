...
func FuzzyFilterKonf(searchTerm string, curItem *store.Metadata, showClusterOnly bool) bool {
...
func NewTableOutputTemplates(maxColumnLen int, showClusterOnly bool) (inactive, active, label string, fmap template.FuncMap) {
	...
	if showClusterOnly {
		inactive = fmt.Sprintf(`  {{ repeat %[1]d " " | print .Cluster | %[2]s }}`, 0, "")
		active = fmt.Sprintf(`▸ {{ repeat %[1]d " " | print .Cluster | %[2]s }}`, 0, "bold | cyan")
		label = fmt.Sprint("  Cluster" + " ")
	}
